...
const OurServices = () => {
  return (
    <>
    <div className="w-full h-full flex bg-[#F3F3F3] pb-10 md:pb-20">
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-20">
          ...
          <div className="lg:col-span-8 flex flex-col space-y-5 w-full lg:max-w-[80%]">
            {services.map((service, index) => (
              <div className="flex flex-col sm:flex-row sm:space-x-4 md:space-x-8 space-y-3 sm:space-y-0 items-start border-b border-[#C4C4C4] py-5" key={index}>
                <div className="flex items-end font-dream-avenue text-[#A3AED0]">
                  <span className='text-3xl sm:text-4xl md:text-[48px] leading-12 font-bold italic '>0{++index}</span>
